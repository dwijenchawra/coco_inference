...
# pipeline_config = 'ssd_mobilenet_v1_fpn_640x640_coco17_tpu-8/pipeline.config'
# model_dir = 'ssd_mobilenet_v1_fpn_640x640_coco17_tpu-8/checkpoint'
...
while (True):
    ...
    frame = frame[int((len(frame) - dimensions) / 2):int(len(frame) - (len(frame) - dimensions) / 2),
            int((len(frame[0]) - dimensions) / 2):int(len(frame[0]) - (len(frame[0]) - dimensions) / 2)]
